...
/**
 * preview.php
 * Preview page for viewing draft or live content
 */

// Get preview parameters
$draft = isset($_GET['draft']) && $_GET['draft'] === 'true';
$preview_mode = true;

// Set page title and description
$title = ($draft ? 'DRAFT Preview - ' : 'LIVE Preview - ') . 'Mannar';
$description = 'Preview mode for Mannar website content';
...
<div class="preview-indicator">
    Vorschau-Modus: <span id="previewMode"><?php echo $draft ? 'ENTWURF' : 'LIVE'; ?></span>
</div>

<!-- Home Section -->
<div class="home-container w3-display-container" id="home" style="margin-top: 30px;">
...
<style>
  ...
  .preview-indicator {
    ...
    font-weight: bold;
    z-index: 1001;
    background-color: <?php echo $draft ? '#ff9800' : '#4CAF50'; ?>;
  }
  
  body {
    margin-top: 30px; /* Make space for the preview indicator */
  }
...
<script>
// Set content type for loading (draft or live)
document.addEventListener('DOMContentLoaded', function() {
  // Modify the content loading function to use draft content if needed
  const contentType = <?php echo $draft ? "'draft'" : "'main'"; ?>;
  
  // This will be handled by main.js, but we set a global flag
  window.previewMode = true;
  window.previewContentType = contentType;
  
  console.log(`Preview mode active: Loading ${contentType} content`);
});
</script>
